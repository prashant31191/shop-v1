...
class ImportController extends Controller
{
    ...
    public function ebaySubSubCategories(){
        ...
        // dd($key->id);
            ...
            $content = file_get_contents("http://open.api.ebay.com/Shopping?callname=GetCategoryInfo&appid=SergheiP-ShopLara-PRD-f44838eb2-53c2fd8f&version=967&siteid=0&CategoryID=$key->id&IncludeSelector=ChildCategories&RESPONSE-DATA-FORMAT=JSON");

            dd($content);

            $subcategories = json_decode(json_encode($content));

            // dd($subcategories->CategoryArray->Category->CategoryName);

            if ($i>$limit) {
                break;
            }
    
            $subcategory_exist = Category::where('name', $subcategories->CategoryArray->Category->CategoryName)->first();
            if ( ($subcategory_exist === null) && ($subcategories->CategoryArray->Category->CategoryID >= 0 ) ) {

                // dd($category_exist);
                $subdubcategory = Category::create([    
                    // 'name' => 'Pop Corn Dulce',
                    'name' => $subcategories->CategoryArray->Category->CategoryName,
                    'category_id' => $key->id,
                    // 'id' => $category->CategoryID,
                ]); 
